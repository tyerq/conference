...
public class Conference {
	...
	private static final List<String> DEFAULT_TOPICS = ImmutableList.of(
			"Default", "Topic");
	...
	public Conference(final long id, final String organizerUserId,
		...
		Preconditions.checkNotNull(conferenceForm.getName(),
	...
	public void updateWithConferenceForm(ConferenceForm conferenceForm) {
		...
		this.topics = topics == null || topics.isEmpty() ? DEFAULT_TOPICS
				: topics;
		this.city = conferenceForm.getCity() == null ? DEFAULT_CITY
				: conferenceForm.getCity();
		...
		if (conferenceForm.getMaxAttendees() < seatsAllocated) {
			throw new IllegalArgumentException(seatsAllocated
					+ " seats are already allocated, "
					+ "but you tried to set maxAttendees to "
	...
	public void giveBackSeats(final int number) {
		if (seatsAvailable + number > maxAttendees) {
			throw new IllegalArgumentException(
					"The number of seats will exceeds the capacity.");
		}
		seatsAvailable = seatsAvailable + number;
	}
	...
	public String toString() {
		...
		stringBuilder.append("Max Attendees: ").append(maxAttendees)
				.append("\n");
